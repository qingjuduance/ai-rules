...
本文件是 `ai-rules` 的通用客户端治理插件事实源，同时也是兼容 `AGENTS.md` 生态的
...
- 合并嵌入式 `ai-rules` 任务 worktree 后，不能只收口 `.codex/ai-rules/`
  子仓库。宿主仓库也必须作为同一条 closeout 链路处理：刷新
  `.codex/project/state/worktrees.json`，检查并提交 `.codex/ai-rules` submodule
  gitlink，更新对应 task tracking，并用
  `worktree-task host-closeout --repo ai-rules --require-task-tracking` 或
...
## ai-rules 客户端治理插件与生命周期组件

- `ai-rules` 是建立在 Codex、Claude Code、Cursor、Cline、Windsurf、Continue 等
...
- 宿主客户端负责对话上下文、工具调用、文件写入、终端执行、人类批准和会话承载；
  `ai-rules` 负责治理这些能力何时可用、如何记录证据、如何检查 worktree/Git/文档边界、
  以及最终输出必须覆盖哪些状态。
- 如果未来自建 agent runner，可以把本节节点映射到 LangGraph 等 workflow 引擎；
  在现有客户端中，`runtime components` 只是 `ai-rules` 插件内部的治理节点注册表，
  不能假设它能拦截客户端内部所有行为。
...
- 文档影响面和引用反查默认由 `gate-pool` 聚合触发一次
  `ai_rules.py doc-index check --changed-path ...`；不要对同一 changed-path 集合重复跑同一节点。
- 文档引用图入口：
...
- 声称任务完成前必须运行或记录 completion test 节点：
  `python .codex/ai-rules/scripts/ai_rules.py completion-test ...`。该节点根据
