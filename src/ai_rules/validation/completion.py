...
def touches_runtime(paths: list[str]) -> bool:
    runtime_prefixes = (
        "src/ai_rules/runtime/",
        "src/ai_rules/lifecycle/",
        "src/ai_rules/gates/",
        "src/ai_rules/worktree/",
        "src/ai_rules/validation/",
    )
...
def planned_checks(task_types: list[str], changed_paths: list[str]) -> list[PlannedCheck]:
    ...
    if suffixes & TEXT_SUFFIXES:
        add(PlannedCheck("validate-encoding", "ai_rules.py validate-encoding --paths <changed-text-files>", "Text files changed."))
    if ".md" in suffixes or "docs" in task_types:
        add(PlannedCheck("validate-doc", "ai_rules.py validate-doc --paths <changed-markdown-files>", "Markdown or docs task in scope."))
        add(PlannedCheck("doc-index", "ai_rules.py doc-index check --changed-path <paths>", "Docs, references, README, or backlinks may be affected."))
    if "rules-script" in task_types or touches_runtime(changed_paths):
        add(PlannedCheck("runtime-components", "ai_rules.py runtime components --format json", "Runtime architecture changed."))
        add(PlannedCheck("gate-pool-dry-run", "ai_rules.py gate-pool --dry-run --final", "Registered gates should be visible and de-duplicated."))
        add(PlannedCheck("selftest", "ai_rules.py selftest --root <target-project>", "Rules/script behavior changed."))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plan completion tests for an ai-rules task.")
