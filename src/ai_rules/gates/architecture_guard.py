...
from ai_rules.common.paths import PROJECT_DIR, PROJECT_RULES_ENTRY, PROJECT_SKILLS_DIR


ALLOWED_CODEX_TOP = {"ai-rules", "ai-rules-config.json", "project"}
LEGACY_CODEX_TOP = {
    "rules",
    "cache",
    "tmp",
...
AI_RULES_SKILLS_DIR = Path(".codex") / "ai-rules" / ".codex" / "skills"
...
def build_report(root: Path) -> dict[str, object]:
    ...
    ai_rules_skills = root / AI_RULES_SKILLS_DIR
    ...
    if ai_rules_skills.exists():
        ai_rules_skill_names = sorted(item.name for item in ai_rules_skills.iterdir() if item.is_dir())
        notes.append(f"ai-rules skills: {len(ai_rules_skill_names)}")
    else:
        ai_rules_skill_names = []

    native_project_duplicates = sorted(set(native_skill_names) & set(project_skill_names))
    native_ai_rules_duplicates = sorted(set(native_skill_names) & set(ai_rules_skill_names))
    project_ai_rules_duplicates = sorted(set(project_skill_names) & set(ai_rules_skill_names))
    ...
    for name in native_ai_rules_duplicates:
        warnings.append(
            Finding(
                "warning",
                "native project skill shadows ai-rules skill; native asset has highest priority and must not be overwritten",
                f"{NATIVE_PROJECT_SKILLS_DIR.as_posix()}/{name}",
            )
        )
    for name in project_ai_rules_duplicates:
        warnings.append(
            Finding(
                "warning",
                ".codex/project skill shadows ai-rules skill; project specialization wins after native assets and conflict must be reviewed",
    ...
    for base, names, label in [
        ...
        (ai_rules_skills, ai_rules_skill_names, "ai-rules"),
    ]:
        for name in names:
    ...
    return {
        ...
        "duplicate_skill_names": sorted(set(native_project_duplicates + native_ai_rules_duplicates + project_ai_rules_duplicates)),
        "duplicate_skill_breakdown": {
            "native_project": native_project_duplicates,
            "native_ai_rules": native_ai_rules_duplicates,
            "project_ai_rules": project_ai_rules_duplicates,
        },
    }
