#!/usr/bin/env python3
"""Unified ai-rules command entry."""
...
from ai_rules.agents import comm as agent_comm
from ai_rules.agents import group_status
from ai_rules.audit import rule_tooling_audit
from ai_rules.docs import doc_index, validate_doc_task
from ai_rules.gates import architecture_guard, gate_pool, session_gate, task_gate
from ai_rules.io import context_extract
...
COMMANDS: dict[str, tuple[str, Callable[[], int]]] = {
    ...
    "architecture-guard": ("Check .codex architecture boundaries.", architecture_guard.main),
    ...
    "rule-audit": ("Audit AGENTS/README sections for tooling migration.", rule_tooling_audit.main),
    ...
    "selftest": ("Run ai-rules black-box self-tests.", selftest.main),
...
def render_commands() -> str:
    lines = ["Available ai-rules commands:"]
...
def main() -> int:
    ...
    if command not in COMMANDS:
        print(f"Unknown ai-rules command: {command}", file=sys.stderr)
        print(render_commands(), file=sys.stderr)
        return 2
    sys.argv = [f"ai_rules {command}", *remainder]
