...
def write_state(con, previous: dict[str, Any] | None, fetched: bool, status: str) -> None:
    now = iso_now()
    data = {
    ...
    state_store.upsert_state(
        con,
        state_type="sync-check",
        state_key="ai-client-governance",
        payload=data,
        source_command="ai_client_governance.py sync-check",
        summary=f"sync-check status={status}",
        event_type="sync_check.state_saved",
    )
...
def check_sync(args: argparse.Namespace) -> SyncReport:
    ...
    db = state_store.db_path(project, args.db)
    con = state_store.connect(db)
    previous_row = state_store.read_state(con, state_type="sync-check", state_key="ai-client-governance")
    state = previous_row["payload"] if previous_row else None
    ...
    if not embedded_repo.exists():
        ...
        write_state(con, state, fetched=False, status=report.status)
        return report
    ...
    write_state(con, state, fetched=report.fetched, status=report.status)
    return report
