...
| 能力网关原则 | `AGENTS.md` 的"客户端治理插件与生命周期组件"章节，记录 file/shell/Todo/approval/agent/final adapters 与 non-invasive raw shell 边界；实施级方案应进入结构化任务/问题记录。 |
...
- **插件化治理模型**：`ai-client-governance` 是人和 AI 之间的客户端治理插件。输入过滤器拆用户要求，
  ...
  不再只追加口头规则。
- **客户端能力网关原则**：运行时治理的下一步收束到 file-write、shell-exec、Todo、
...
- **多 Agent 审批结论**：多 Agent 执行链路中，reviewer Agent 必须按 executor Agent
  的 task id/leaf id 给出当前是否通过的结构化结论，记录双方 client_type、未处理项、
  处理不佳项、修改建议和复测结果；结论为不通过时不能合并。
...
只读并行批次、验证批次和必须顺序执行的状态变更步骤，并为每条命令输出
`capability`、`risk_level`、`side_effect`、`cache_eligible`、`parallel_eligible`
和 `approval_required`，同时附带统一 policy 的 `policy_decision`、`policy_severity`
和 `policy_findings`。`task-run run` 默认在 subprocess 执行前阻断 `block` 或未带明确
`--policy-approval-label` 的 `approval_required` 命令；这把启发式 capability 事实接到
可审计的本地 policy gate 上。可直接入库的事件 payload 必须包含非空 `groups`、
`command_classifications` 和 policy 决策，否则后续门禁没有足够证据判断命令风险。
...
`span_kind=command`、`subject_type=command` 的一种执行载荷；未来模型 HTTP、委派 Agent、
...
`-NoProfile` 运行，不写用户 `$PROFILE`。需要强制覆盖时运行
`task-run diagnose --require-raw-shell-coverage` 或
`shell-adapter diagnose --require-raw-shell-coverage`。
这不是替代宿主客户端，而是在命令进入宿主 shell 前提供强制过滤器、执行器和诊断层，
减少多轮模型 HTTP 往返。
诊断默认可看全局 telemetry，也可用 `--task-id`、`--trace-id`、`--since`、`--until`
收敛到当前任务，避免历史失败或重复命令淹没本轮信号。
`task-queue lifecycle` 提供 task queue 与 structured task record 的统一视图，
把 queue 的 `completed` 和 task record 的 `done` 归一为 lifecycle `done`，同时报告
缺失、状态漂移和 trace_id 漂移；`--fail-on-drift` 可以把这些漂移变成门禁失败。
需要推进生命周期时用 `task-queue transition --task-id <id> --to done`，它显式同步 queue
和 task-record 并写入 `task-queue.transition` 事件，而不是让只读视图隐式写回。
...
AI 执行修改型、规则/脚本、docs、git、correction、multi-agent 或 long-running 任务时，
按下面顺序走治理链路。某一步缺事实时，不进入后一步。

1. `sync-check`：确认嵌入式治理仓库存在且同步状态可见。
2. `lifecycle input-filter`：拆 REQ、scope、claim、client/model identity、agent 决策、
   readonly side-effect、shell proxy 计划和 history recovery。
3. `task-queue enqueue` / `task-queue transition --to active`：记录根任务，保持一次一个
   active 根任务；并行只能在根任务内部拆 task-node/worktree/agent。
4. `task-record apply`：写入 SQLite task facts；中/大型和修改型任务缺 task record 时
   不允许进入 write-intent。
5. `worktree-task create`：写仓库前创建任务 worktree；通用治理变更使用
   `--repo ai-client-governance`。
6. `task-record gate --event preflight` 或 `lifecycle preflight`：验证分析契约、审批、
   worktree、command-compression、scope、agent、history、readonly 和 shell proxy facts。
7. `task-run plan/run`、`gate-pool`、`shell-adapter proxy-powershell` 或
   `tool-invocations run`：确定性本地命令走治理入口并写 telemetry；Git 写入、锁、
...
9. `task-record gate --event final`：检查 requirements、outputs、worktree、validation、
   telemetry、command-error、discovered issue facts，以及 multi-agent 任务的 executor Agent task id
   通过/不通过结论事实。
...
11. `task-queue transition --to done`：只在 task record 存在、final gate 通过、live
    worktree 状态复核且无未解释 dirty/command-error/raw-shell gap 后执行。

`task-queue lifecycle --fail-on-drift` 用来检查 queue 与 task-record 是否一致。
如果出现 `missing_in_task_record`、trace 漂移或 queue 已 completed 但 task record 不存在，
不能声称任务完成，也不能把 AI 自述当作事实源。

当用户明确要求"只写方案，交给另一个 AI 实现，再由另一个 AI 验收"时，这仍然是一条
治理生命周期。当前 AI 要把 design package 写入 task record，覆盖目标、非目标、数据模型、
policy/gate、迁移/清理、验证计划、风险、implementation tasks、review acceptance criteria
和 handoff capsule。executor Agent 读取 design package 后写实现 facts；reviewer Agent 必须独立运行
验证和 live-state 检查，不能只复述 executor Agent 的结论。设计包、执行任务和验收任务通过
...
当执行链路包含多个 Agent 时，reviewer Agent 必须按 executor Agent 的 task id 或 leaf id 写入
`agent-review-result.analysis` 结构化事实。验收记录至少包含 reviewer agent/client_type、
executor agent/client_type、reviewed task id 或 leaf id、pass/fail 结论、生命周期事实核对、
提交状态核对、未处理项、处理不佳项、证据、整改建议、复测计划和复测结果。生命周期核对必须读取 task queue
...
`host-closeout` 或 `task-queue transition --to done`。当前已由 `task-gate`、runtime
components 和 `task-record gate --event final` 检查 Markdown 表格与 DB 结构化事件；
`worktree-task closeout-all`、`finalize`、`host-closeout` 和 agent-groups/agent-comm 看板
仍是后续接入点，不能把这些点声称为已经全链路强制。
...
如果旧 Markdown、旧 pending、旧 corrections 汇总或聊天记录还写着"当前任务"、
"active/pending"或"已完成"口径，AI 只能把它们当审计材料或一次性 cleanup 输入。
当前任务和问题必须从 SQLite、`task-queue lifecycle`、`worktree-task status --record-state`
和 Git live state 重新确认；发现旧人读状态会误导后续客户端时，记录 cleanup 任务或在当前
范围内删除/替换，不新增兼容读取。
...
| CodeBuddy | `.codebuddy/rules/ai-client-governance/RULE.mdc` | 仅在 `-InstallAgentAdapters` 或 `-ForceAgentAdapters` 时生成；CodeBuddy 在无 `CODEBUDDY.md` 时会自动加载根 `AGENTS.md`，此 adapter 作为 `.codebuddy/rules` 目录形式的专用入口，带 YAML frontmatter（`alwaysApply: true`），用户原生规则保留。 |
...
Cursor rules、Cline rules、Continue rules、Devin rules、Aider conventions
和腾讯云代码助手 CodeBuddy rules（`.codebuddy/rules/<rule>/RULE.mdc`，根 `AGENTS.md` 在无 `CODEBUDDY.md` 时被自动加载）。
...
`session-bootstrap` 默认只输出根入口、通用规则和项目规则的行数、字节数、SHA256、
有限标题索引、sync-check 摘要和下一步安全命令；不会把长 `AGENTS.md` 正文整段写到终端。
需要细读某一段时，再用下面的 `context-extract --headings`、`--match` 或 `--range`。
...
会话启动时先用 `session-bootstrap`，后续深读再用 Python 摘录脚本读取标题索引、
关键词命中段或明确行号范围：
...
而不是按节点或按文件反复慢跑。
文档影响面使用事件冒泡：从 changed file 所在目录向同目录 README、`.references/`、上级
README/AGENTS、manifest、命令说明和跨目录入链扩散；只有出现入链、断链、缺锚点、
入口职责变化或规则边界变化时扩大到全局。冒泡结果要进入 task record 的 doc-impact、
validation 或 no-impact facts，不能只在最终回复说"检查过文档"。

registry 中的 `security-policy`、`raw-shell-coverage`、`task-run-diagnostics`、
`shell-adapter-diagnostics` 和 `task-record-queue-alignment` 现在都会被 `gate-pool`
转成真实步骤。这里的 raw shell 设计刻意是 fail-closed 诊断，而不是声称本仓库能拦截
Codex、Claude、Cursor 等宿主客户端内部所有裸 shell；可强制的边界是经过
`task-run`、`gate-pool`、`shell-adapter`、`tool-invocations` 的命令路径。Windows
PowerShell 场景必须用 `shell-adapter proxy-powershell` 形成 no-profile command proxy
证据；`task-run diagnose --require-raw-shell-coverage` 会在既没有自动拦截、
也没有 command proxy 证据时失败。
...
wrapped telemetry 是补偿证据，不等同于 shell-adapter auto-intercept；no-profile
command proxy 是当前 Windows PowerShell 的强制 raw-shell 覆盖证据，不写用户 `$PROFILE`。
...
命令错误也要成为结构化事实。非预期失败的命令需要写入
`events.event_type=command-error.analysis` 或等价 fact，包含 failed command、exit code、
phase、shell/parser、failure category、root cause、corrected command、retry count、
...
给出 command-file 预警；`JSON 输出 | python -c "json.load(sys.stdin)"` 这类后处理会归类为
`pipeline_python_c_json_postprocess` 并在 PowerShell proxy 中默认阻断。需要抽取 JSON 字段时使用：

```powershell
python .ai-client\ai-client-governance\scripts\ai_client_governance.py json-query `
  --path items[].item_id `
  -- python .ai-client\ai-client-governance\scripts\ai_client_governance.py framework-debt list --format json
```

`telemetry report --summary --format json` 只返回 span/失败/command-error/cache/top operations/
slowest validation 等短摘要；需要完整取证时再去掉 `--summary`。`tool-flow --format json`
默认只输出 compact invocation，需要取证 full payload 时显式加 `--include-raw-json`。
...
这类状态变更必须再经过 `policy assess`、`task-run run` 或 `worktree-task`/`host-closeout`
链路。裸 git 写命令只能作为 break-glass，并要记录批准标签、原因、影响路径和补偿验证。
默认命令和诊断脚本解析只认嵌入式 `.ai-client/ai-client-governance/` 或治理仓库自身入口；
宿主根目录的 `scripts/ai_client_governance.py` 不作为旧路径 fallback。

`runtime manifest-report --check-manifest` 会把代码 registry 里的 component kinds、
component contract 和核心 runtime commands 与 `manifest.json` 对比。这个报告先作为
声明式防漂移门禁，不马上把 registry 全量改成 manifest 生成；等后续架构窗口再把
manifest 升级为生成源，当前 report 可以先持续证明 README/manifest/registry 没有继续手工漂移。
...
委派 Agent 用于验收门禁时，输出不能只写"通过"。task tracking 需要有
`## 多 Agent 验收矩阵`，逐行覆盖 REQ、门禁、失败路径、成功路径、发现问题、修复复测和
执行客户端，确保 Agent 测的是整条生命周期链，而不是随机抽样。`task-gate` 会按矩阵行解析，
只有一句话式"全面覆盖"不能替代结构化矩阵；旧 `子 AI 验收矩阵` 不再作为有效入口。

多 Agent 任务还需要 `## 多 Agent 审批结论` 或 DB 中
`agent-review-result.analysis` 事实。验收表按 executor Agent 的 task id/leaf id 逐行记录：
双方 client_type、是否通过、生命周期事实是否闭合、提交状态是否闭合、未处理 REQ、
处理不佳内容、证据、修改建议和复测方式。这个结论不是主观评价，而是合并门禁：
不通过、生命周期记录与 Git live state 冲突、未处理项未解释、整改建议为空或复测失败时，
最终 gate 必须阻止合并和 done 状态。closeout/host-closeout 和看板联动仍按后续接入点记录。
...
- multi-agent 任务是否已有 reviewer Agent 对 executor Agent task id 的通过结论；未通过时写明退回或阻塞原因。
...
`task-queue transition --to done` 只能在这些事实都存在后执行：task record 存在并可通过
final gate，requirements 已关闭，worktree live state 由 `worktree-task status --record-state`
或 `finalize --record-state` 更新，验证和 telemetry facts 可查，没有未解释的 dirty
worktree、未跟踪运行态产物、policy-blocked 命令、command-error 或 raw shell gap。
...
- AI 协作审批、任务量评估、恢复现场、Git 边界和多 Agent 协作。
...
python .ai-client\ai-client-governance\scripts\ai_client_governance.py shell-adapter proxy-powershell `
...
`selftest` 默认输出紧凑摘要：每个测试只显示 PASS/FAIL、summary 和命令数量；
需要排查失败时再加 `--verbose` 展开每个子命令和 stdout/stderr 首行。
它默认把 run directory 放到 `.ai-client/project/tmp/ai-client-governance-selftest/`
...
这些残留在任务 worktree 中出现时也是 dirty blocker：不能把包含未跟踪 selftest、doc-index、
pycache、cache 或 lifecycle 产物的 worktree 标记为 completed，除非 artifact manifest
明确允许并记录 cleanup/reconcile 策略。
