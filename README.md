...
| Token 用量统计 | 嵌入后 `.codex/ai-rules/.codex/skills/codex-token-usage/` |
...
3. 更新目标项目根 `AGENTS.md` 为薄入口：先读 `.codex/ai-rules/AGENTS.md`，
   再读 `.codex/project/rules/project/AGENTS.md`。
4. 确认项目特有规则只放在 `.codex/project/rules/project/`，不要写回本仓库。
...
git add .gitmodules .codex/ai-rules AGENTS.md `
  .codex/ai-rules-config.json .codex/project/rules/project/AGENTS.md
...
- `AGENTS.md`：目标项目拥有的入口文件，通常只承载读取顺序和边界说明。
...
- 本地 Codex session token 用量统计 skill。
...
manifest、scripts 和 skills 的唯一通用事实源。安装脚本只生成根 `AGENTS.md`、
`.codex/ai-rules-config.json` 和缺失时的项目规则占位，不再声明或写出旧 common
副本、scripts 副本或 skills 副本目标。

迁移旧项目时，先把完整仓库嵌入 `.codex/ai-rules/`，再把根 `AGENTS.md` 改为读取
`.codex/ai-rules/AGENTS.md` 和 `.codex/project/rules/project/AGENTS.md`。旧的 common
副本、脚本副本或 skill 副本不参与新模型的读取、安装和同步。

如果目标项目已经有 `AGENTS.md` 或本地 skills，先把它们当作原生项目事实源管理。
`ai-rules` 只能补缺、索引和报告冲突，不能静默覆盖；根据纠错或项目流程生成的
项目特化 skill 统一维护在 `.codex/project/skills/`，通用 skill 保持在
`.codex/ai-rules/.codex/skills/`。
