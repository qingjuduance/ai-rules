...
这不是替代宿主客户端执行命令，而是在命令进入宿主 shell 前先做确定性压缩，减少多轮模型
HTTP 往返。
...
去重、工具账本、completion-test 和 selftest；这能发现“没记录输入”“缺 worktree 证据”
“最终门禁没跑”等硬问题。仍需继续优化的结构是：把 `task-run plan` 扩展为可执行 DAG runner，
用 `HEAD + changed-path hash + task types + command args + script version` 作为缓存键，
并把 stale coord session、重复验证、未入账裸 shell 命令作为自动诊断报告，而不是等人工发现。
