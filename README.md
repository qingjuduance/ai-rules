...
`risk_flags` 和 `verification_action`；凡会影响写入、Git、脚本账本或规则升级的陈述，
...
每个执行节点默认写入 `tool-invocations` JSONL 账本，并可用 `--trace-json` 输出完整
trace。`task-run diagnose` 读取同一账本和 worktree-coord 状态，报告重复终态命令、
失败、cache hit/miss、活动锁和裸 shell 自动拦截缺口。这不是替代宿主客户端，而是在命令
进入宿主 shell 前提供强制过滤器、执行器和诊断层，减少多轮模型 HTTP 往返。
诊断默认可看全局账本，也可用 `--task-id`、`--trace-id`、`--since`、`--until`
...
`.ai-client/project/records/corrections/`、`.ai-client/project/logs/tool-invocations/`
或业务文档写回本仓库。
...
owner/cleanup 链路，再运行脚本恢复；不要手工编辑 runtime ledger、coord state 或 lock
来掩盖脚本 bug。
...
修改型任务不能继续收口。当前本地命令账本已经能记录 `gate-pool` 子门禁和显式
`tool-invocations run/record`，但宿主客户端裸 shell 调用不能被本仓库自动拦截，所以重要命令
要通过 wrapper 执行或在 task record 说明例外。

当前自我检测机制已经有效覆盖输入拆解、结构化 task record、worktree live state、gate-pool
去重、工具账本、completion-test、task-run DAG/cache/diagnose 和 selftest；这能发现
...
session、重复验证、失败账本和裸 shell 拦截缺口做成自动诊断报告，而不是等人工发现。
...
能证明本地命令压缩、账本和 cache 行为，`gate-pool --dry-run`
