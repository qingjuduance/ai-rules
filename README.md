...
和 `approval_required`。这些字段先作为 planner 和 task record 的稳定事实输入，
不在同一变更里升级为强阻断策略，避免启发式误判直接打断任务。可直接入库的事件 payload
必须包含非空 `groups` 和 `command_classifications`，否则 `task-record gate` 不会放行。
...
重复 subject 和 gate/completion/final-gate 数量差异；`task-run diagnose`
...
`task-queue lifecycle` 提供 task queue 与 structured task record 的只读统一视图，
把 queue 的 `completed` 和 task record 的 `done` 归一为 lifecycle `done`，同时报告
缺失、状态漂移和 trace_id 漂移。它是迁移前的统一读取模型，不替两套状态做隐式写回。
