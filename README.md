...
| 机器清单 | `manifest.json` |
| 嵌入脚本 | `install-ai-client-governance.ps1`，一键嵌入、生成目标项目配置，并按缺失补齐常见入口 adapter。 |
| 会话检查 | `check-ai-client-governance-sync.ps1` |
| 统一 CLI | `python .codex/ai-client-governance/scripts/ai_client_governance.py --list` |
| Python 包 | `.codex/ai-client-governance/src/ai_client_governance/` |
| 修改工作区 | 所有修改型任务先创建 `.codex/project/.worktree/<task-slug>/` 任务 worktree。 |
| Token 用量统计 | 嵌入后 `.codex/ai-client-governance/.codex/skills/codex-token-usage/`，这是 Codex 本地日志的可选 skill，不代表框架只服务 Codex。 |
| 推荐嵌入方式 | Git submodule |
| 默认同步策略 | 每次会话检查；最多 24 小时 fetch 一次；不自动 pull/push。 |
| 回写方式 | 进入 `.codex/ai-client-governance/` 后使用普通 Git 命令提交和推送。 |
...
- **完整 Git 边界**：通用规则有自己的 commit、branch、remote 和 history；
  ...
  可以直接在 `.codex/ai-client-governance/` 用 Git 回写。
...
- **多入口 adapter 目录**：安装脚本可以在缺失时生成 `AGENTS.md`、`CLAUDE.md`、
  `GEMINI.md`、`.github/copilot-instructions.md`、`.cursor/rules/ai-client-governance.mdc`、
  `.clinerules/ai-client-governance.md`、`.windsurf/rules/ai-client-governance.md`、
  `.continue/rules/ai-client-governance.md`、`.roo/rules/ai-client-governance.md` 和 `CONVENTIONS.md`。
  已存在的项目原生入口默认保留，不静默覆盖。
- **只支持完整嵌入模型**：目标项目把本仓库整体放在 `.codex/ai-client-governance/`，
  不再生成或维护 common 规则、scripts、skills 的分散复制副本。
- **三层资产边界清楚**：优先级固定为目标项目原生资产 >
  `.codex/project/` 项目特化层 > `.codex/ai-client-governance/` 通用层。本仓库只维护跨项目
...
- **插件化治理模型**：`ai-client-governance` 是人和 AI 之间的客户端治理插件。输入过滤器拆用户要求，
  ...
  不再只追加口头规则。
- **门禁池和调用链路**：`ai_client_governance.py gate-pool` 可以按同一 `trace_id` 编排多个
  门禁，`ai_client_governance.py tool-invocations` 和 `ai_client_governance.py tool-flow` 记录并验证调用链路。
...
- **核心事实源**：`.codex/ai-client-governance/AGENTS.md` 和
...
| GitHub Copilot | `.github/copilot-instructions.md`、`.github/instructions/ai-client-governance.instructions.md` | 默认只在缺失时生成，避免覆盖团队已有 Copilot 指令。 |
| Cursor | `.cursor/rules/ai-client-governance.mdc` | 使用 `alwaysApply: true` 的项目规则 adapter，已存在则保留。 |
| Cline | `.clinerules/ai-client-governance.md` | Workspace rules adapter，已存在则保留。 |
| Windsurf | `.windsurf/rules/ai-client-governance.md` | Project rules adapter，已存在则保留。 |
| Continue | `.continue/rules/ai-client-governance.md` | Project-specific rules adapter，已存在则保留。 |
| Roo Code | `.roo/rules/ai-client-governance.md` | Project rules adapter，已存在则保留。 |
| Aider | `CONVENTIONS.md` | Aider convention adapter；已存在时保留，因为它常承载项目真实约定。 |

这个清单是兼容层，不是优先级反转。目标项目已有的原生规则文件最高优先级；
`.codex/project/` 是项目特化层；`.codex/ai-client-governance/` 是通用层。adapter 只负责把不同
...
2. 在目标项目中嵌入完整仓库，推荐路径为 `.codex/ai-client-governance/`；
...
4. 确认所有 adapter 都指向 `.codex/ai-client-governance/AGENTS.md` 和
   `.codex/project/rules/project/AGENTS.md`，且项目特有规则不写回本仓库。
5. 每次新会话先运行 `.codex/ai-client-governance/check-ai-client-governance-sync.ps1` 或目标项目根部
...
```powershell
git submodule add <ai-client-governance-url> .codex/ai-client-governance
...
```powershell
git -c protocol.file.allow=always submodule add <local-ai-client-governance-path> .codex/ai-client-governance
git submodule update --init --recursive
```
...
```powershell
git -C .codex/ai-client-governance fetch origin
git -C .codex/ai-client-governance pull --ff-only
git add .gitmodules .codex/ai-client-governance
...
```powershell
git clone <ai-client-governance-url-or-local-path> .codex/ai-client-governance
```

这种方式更轻，但父仓库不会自然记录嵌入规则版本。需要在目标项目 README、
`.codex/ai-client-governance-config.json` 或 task tracking 中记录当前 commit。
...
- 把完整 `ai-client-governance` 仓库嵌入到 `.codex/ai-client-governance/`，Git 项目默认用 submodule。
...
- 写 `.codex/ai-client-governance-config.json`，记录 source URL、submodule path、branch
  同步策略和 adapter catalog。
- 安装后运行 `.codex/ai-client-governance/check-ai-client-governance-sync.ps1 -NoFetch` 做本地一致性检查。
...
- `CLAUDE.md`、`GEMINI.md`、`.github/copilot-instructions.md`、
  ...
  `-ForceAgentAdapters`。
- `.codex/ai-client-governance-config.json` 已存在：默认备份后重写，保持配置事实源最新。
...
- `.codex/project/skills/` 是项目特化层：用于放置根据 ai-client-governance 流程、纠错和当前项目
  经验生成的 skill，不用于承接通用 skill 的复制副本。
- skill 同名时按“原生项目 skill > `.codex/project` skill > `ai-client-governance` skill”选择，
  后续由架构守卫或人工 review 报告冲突。
- `.codex/ai-client-governance/` 已存在且是已注册 submodule：不重复添加，只继续配置检查。
- `.codex/ai-client-governance/` 已存在但不是 Git 仓库：停止并提示用户手工处理。
- `.codex/ai-client-governance/` 是 Git 仓库但未注册为 submodule：默认停止；确认要接管时，
...
git status
git diff --cached
git add .gitmodules .codex/ai-client-governance AGENTS.md `
  CLAUDE.md GEMINI.md CONVENTIONS.md .github .cursor .clinerules `
  .windsurf .continue .roo .codex/ai-client-governance-config.json `
...
```powershell
python .codex\ai-client-governance\scripts\ai_client_governance.py --list
python .codex\ai-client-governance\scripts\ai_client_governance.py task-gate --help
...
```powershell
powershell -ExecutionPolicy Bypass -File .\.codex\ai-client-governance\check-ai-client-governance-sync.ps1 `
  -TargetProjectPath .
```
...
```powershell
python .\.codex\ai-client-governance\scripts\ai_client_governance.py sync-check --target-project-path .
```

检查脚本只读检查嵌入仓库状态，默认行为如下：

- 每次会话都检查 `.codex/ai-client-governance` 是否存在、是否为 Git 仓库、是否有 dirty 改动。
...
- warning 会每次出现，直到用户在 `.codex/ai-client-governance/` 中完成同步。
...
```powershell
python .\.codex\ai-client-governance\scripts\ai_client_governance.py context-extract `
...
```powershell
git -C .codex/ai-client-governance status
git -C .codex/ai-client-governance pull --ff-only
git -C .codex/ai-client-governance push
```
...
```powershell
python .codex\ai-client-governance\scripts\ai_client_governance.py worktree-task create `
...
python .codex\ai-client-governance\scripts\ai_client_governance.py worktree-task status --write-state
python .codex\ai-client-governance\scripts\ai_client_governance.py worktree-task status --format json
python .codex\ai-client-governance\scripts\ai_client_governance.py worktree-task close `
  --repo ai-client-governance `
  --task-slug worktree-task-script
...
宿主仓库记录的 gitlink。这个收口不能只在 `.codex/ai-client-governance/` 子仓库内完成：
...
git add .codex\ai-client-governance .codex\project\state\worktrees.json `
...
`host-closeout` 会比较宿主 index 中 `.codex/ai-client-governance` 的 gitlink、嵌入仓库当前 HEAD、
`.codex/project/state/worktrees.json` 记录的 ai-client-governance HEAD，以及相关 task tracking
...
git -C .codex\ai-client-governance worktree add `
  .codex\project\.worktree\ai-client-governance-<task-slug> `
...
`ai_client_governance.py tool-invocations` 写入 `.codex/project/logs/tool-invocations/*.jsonl`，并共享同一个
`trace_id`。
...
python scripts\ai_client_governance.py gate-pool `
  ...
  --task-type rules-script `
  --changed-path .codex\ai-client-governance\AGENTS.md `
  --changed-path .codex\ai-client-governance\src\ai_client_governance\gates\task_gate.py `
  --final
```
...
`.codex/ai-client-governance/.codex/skills/codex-token-usage/` 提供本地只读
...
```powershell
python .codex\ai-client-governance\.codex\skills\codex-token-usage\scripts\codex_token_usage.py `
  --days 30 `
...
schema 3 只描述完整嵌入模型：`.codex/ai-client-governance/` 是通用规则、README、
manifest、scripts 和 skills 的唯一通用事实源。安装脚本只生成薄入口 adapter、
`.codex/ai-client-governance-config.json` 和缺失时的项目规则占位，不再声明或写出旧 common
...
迁移旧项目时，先把完整仓库嵌入 `.codex/ai-client-governance/`，再让目标项目已有入口或新生成
adapter 指向 `.codex/ai-client-governance/AGENTS.md` 和
...
等原生规则或本地 skills，先把它们当作原生项目事实源管理。`ai-client-governance` 只能补缺、
索引和报告冲突，不能静默覆盖；根据纠错或项目流程生成的项目特化 skill 统一维护在
`.codex/project/skills/`，通用 skill 保持在 `.codex/ai-client-governance/.codex/skills/`。
...
$pyFiles = Get-ChildItem -Recurse -Filter *.py .codex\ai-client-governance\scripts, .codex\ai-client-governance\src
...
python .codex\ai-client-governance\scripts\ai_client_governance.py validate-encoding `
  --paths .codex\ai-client-governance\AGENTS.md `
  .codex\ai-client-governance\README.md `
  .codex\ai-client-governance\manifest.json `
  AGENTS.md CLAUDE.md GEMINI.md CONVENTIONS.md `
  .github .cursor .clinerules .windsurf .continue .roo `
  .codex\ai-client-governance\scripts `
  .codex\ai-client-governance\src `
  --require-paths
python .codex\ai-client-governance\scripts\ai_client_governance.py session-gate --help
python .codex\ai-client-governance\scripts\ai_client_governance.py task-gate --help
python .codex\ai-client-governance\scripts\ai_client_governance.py tool-flow --help
python .codex\ai-client-governance\scripts\ai_client_governance.py gate-pool --help
python .codex\ai-client-governance\scripts\ai_client_governance.py worktree-coord --help
python .codex\ai-client-governance\scripts\ai_client_governance.py selftest --root .
```
